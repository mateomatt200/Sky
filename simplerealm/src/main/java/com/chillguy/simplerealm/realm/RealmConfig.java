...
public class RealmConfig {
    ...
    public void updateRealm(Realm realm) {
        ...
        config.set("realms." + realm.getOwner().getUniqueId() + ".money", realm.getMoney());
        config.set("realms." + realm.getOwner().getUniqueId() + ".rank", realm.getRank());
        config.set("realms." + realm.getOwner().getUniqueId() + ".gen_level", realm.getGenLevel().getNivel());
        updateVote(realm);
    ...
    public void setMoney(Realm realm) {
        config.set("realms." + realm.getOwner().getUniqueId() + ".money", realm.getMoney());
        try {
            config.save(file);
        } catch (Exception ex) {
    ...
    public void loadRealm(String name) {
        ...
        int level_gen = config.getInt("realms."+ name +".gen_level", 0);
        GenLevel genLevel = Main.getInstance().getRealmMap().getGenLevels().get(level_gen);
        Realm realm = new Realm(RealmPlayer.getPlayer(config.getString("realms." + name + ".owner.uuid")), ThemeType.themeTypes.get(config.getString("realms." + name + ".theme.id")), spawn, config.getInt("realms." + name + ".level"), config.getInt("realms." + name + ".vote"), config.getString("realms." + name + ".rank"), genLevel);
        for (String s : config.getConfigurationSection("realms." + name + ".players").getKeys(false)) {
        ...
        try {
            realm.addMoney(config.getInt("realms."+name+".money"));
