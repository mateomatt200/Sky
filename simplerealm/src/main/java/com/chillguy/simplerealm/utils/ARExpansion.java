...
public class ARExpansion extends PlaceholderExpansion {
        ...
        public String onPlaceholderRequest(Player player, String identifier){
            ...
            if(identifier.equals("realm_nextlevelprice")){
                if(realm == null)
                    return "0";
                else if(realm.getLevel() == Main.getInstance().getRealmLevel().getRealmByRankAndLevel(realm.getLevel().getRank(), Main.getInstance().getRealmLevel().getRealmLevels(realm.getLevel().getRank()).size()))
                    return "max";

                return ""+ Main.getInstance().getRealmLevel().getRealmByRankAndLevel(realm.getLevel().getRank(),realm.getLevel().getNumber()+1).getPrice();
            }
