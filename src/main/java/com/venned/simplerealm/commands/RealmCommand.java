...
import com.venned.simplerealm.gui.WholeGUI;
        ...
        if (cmd.getName().equalsIgnoreCase("realm")) {
            ...
            if (args.length == 2) {
                if (args[0].equalsIgnoreCase("invite")) {
                    ...
                    if (!player.hasPermission("realm.addmoney")) {
                        player.sendMessage("§cYou don't have the permission to do this.");
                        return false;
                    }
                    if(args[1].matches("\\d+")) {
                        int cmoney = Integer.parseInt(args[1]);
                        try {
                            if (Main.getInstance().getBalance(player) < cmoney) {
                                player.sendMessage("§cYou don't have enough money to do this");
                                player.sendMessage("§cYou have §6" + Main.getInstance().getBalance(player) + " §cand you need §6" + cmoney);
                                return false;
                            }
                        } catch (UserDoesNotExistException e) {
                            throw new RuntimeException(e);
                        }
                        new WholeGUI().openAllMoneyGUI(player, cmoney);
                    }
                    else
                        player.sendMessage("§cWrong command /realm addmoney <amount>");
                }
                ...
                if (args[0].equalsIgnoreCase("top")) {
                    ...
                    player.sendMessage("                      §eRealms commands:" +
                            ...
                            "\n§2/realm addmoney §o<amount>: §r§aAdd money to someone's realm"+
