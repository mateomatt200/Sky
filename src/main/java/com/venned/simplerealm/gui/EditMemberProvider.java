...
public class EditMemberProvider implements InventoryProvider {
    ...
    private void setUpItems() {
        ban = ClickableItem.of(new ItemsUtils(Material.GLASS, "§4Ban "+vised.getName(), Collections.singletonList("§7Ban " + vised.getName() + " from this realm")).toItemStack(), e -> {
            e.setCancelled(true);
        ...
        retour = ClickableItem.of(new ItemsUtils(Material.BLACK_BED,"⬅ §bGo back",Arrays.asList("","§7Click to go back to the","§7Realm options.")).toItemStack(),e ->{
            e.setCancelled(true);
            player.closeInventory();
