...
public class RemoveProvider implements InventoryProvider {
    ...
    private void setUpItems() {
        yes = ClickableItem.of(new ItemsUtils(Material.GRAY_STAINED_GLASS, "§cUnclaim Realm",(byte) 5, Arrays.asList("§7Unclaiming a realm will ", "§7remove your Realm and any ","§7progress on that Realm.")).toItemStack(), e -> {
            e.setCancelled(true);
        ...
        no = ClickableItem.of(new ItemsUtils(Material.GRAY_STAINED_GLASS_PANE, "§aKeep Realm",(byte) 14, Collections.singletonList("§7Cancel unclaim request.")).toItemStack(), e -> {
            e.setCancelled(true);
            e.getWhoClicked().closeInventory();
        });
