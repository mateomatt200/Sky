...
public class ThemeProvider implements InventoryProvider {
        ...
        public void init(Player player, InventoryContents inventoryContents) {
            ClickableItem basic = ClickableItem.of(new ItemStack(Material.GREEN_STAINED_GLASS_PANE, 1, (byte) 15), e -> e.setCancelled(true));
        ...
        private void setItem(InventoryContents inventoryContents, ClickableItem clickableItem) {
            ...
            while (!iterator.ended()) {
                if (iterator.get().isPresent()) {
                    if (iterator.get().get().getItem().getType() == Material.GREEN_STAINED_GLASS_PANE) {
